...
def compute_transition_probs(distance, p_outbred_to_inbred, p_inbred_to_outbred):
    """Compute transition probabilities for a given distance."""
    p_inbred_to_inbred = 1 - np.exp(-distance * p_inbred_to_outbred)
    p_outbred_to_outbred = 1 - np.exp(-distance * p_outbred_to_inbred)
    return p_inbred_to_inbred, 1 - p_inbred_to_inbred, 1 - p_outbred_to_outbred, p_outbred_to_outbred
...
def forward_algorithm(positions, genotypes, ref_freqs, p_outbred_to_inbred, p_inbred_to_outbred):
    ...
    for t in range(1, num_positions):
        ...
        p_inbred_to_inbred, p_inbred_to_outbred, p_outbred_to_inbred, p_outbred_to_outbred = compute_transition_probs(
            distance, p_outbred_to_inbred, p_inbred_to_outbred)
...
def optimize_with_nelder_mead(positions, genotypes, ref_freqs):
    ...
    initial_guess = [TRANSITION_OUTBRED_TO_INBRED, TRANSITION_INBRED_TO_OUTBRED]
    return nelder_mead(objective_function, initial_guess)

def main(vcf_file):
    positions, individuals, genotypes = parse_vcf(vcf_file)
    ref_freqs = [0.5] * len(positions)  # Placeholder reference allele frequencies
